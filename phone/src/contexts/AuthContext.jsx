import AsyncStorage from "@react-native-async-storage/async-storage";
import {
  createUserWithEmailAndPassword,
  sendPasswordResetEmail,
  signInWithEmailAndPassword,
  signOut,
} from "firebase/auth";
import { doc, getDoc, setDoc } from "firebase/firestore";
import { createContext, useEffect, useState } from "react";
import { useToast } from "../hooks/useToast";
import { authentication, db } from "../services/firebase-config";
import { AuthErrorHandler } from "../utils/handleFirebaseError";
import { isStringEmpty } from "../utils/string";

export const AuthContext = createContext({});

export function AuthContextProvider(props) {
  const [user, setUser] = useState();
  const { addToast } = useToast();

  const [isLogged, setIsLogged] = useState(false);
  const [isMenuHide, setIsMenuHide] = useState(false);

  async function getKeyLocalStorage(key) {
    return (await AsyncStorage.getItem(key)) === null
      ? false
      : await AsyncStorage.getItem(key);
  }
  async function setKeyLocalStorage(key, value) {
    return await AsyncStorage.setItem(key, value);
  }
  async function removeKeyLocalStorage(key) {
    const hasLocalStorageData = !((await AsyncStorage.getItem(key)) === null);

    if (hasLocalStorageData) {
      await AsyncStorage.removeItem(key);
    }
  }

  async function updateUserByID(id, data) {
    const userData = await getUserByID(id);

    const userUpdated = {
      ...userData,
      ...data,
    };
    try {
      await setDoc(doc(db, "users", id), userUpdated);

      setUser(userUpdated);

      return true;
    } catch (error) {
      console.error("updateUserByID error", error);
      addToast({
        severity: "error",
        message: `Houve um erro ao atualizar dados do usuário. Tente novamente mais tarde`,
      });
      return false;
    }
  }

  function logout() {
    signOut(authentication)
      .then(async () => {
        setUser(null);
        setIsLogged(false);
        removeKeyLocalStorage("UID");
        await AsyncStorage.clear();
      })
      .catch((error) => {
        addToast({
          severity: "error",
          message: `Houve um erro ao tentar sair. Tente novamente mais tarde`,
        });
        console.log("logout error", error);
      });
  }

  function handleForgotUser(email) {
    if (isStringEmpty(email)) {
      addToast({
        severity: "error",
        message: "O campo email não foi preenchido",
      });
      return true;
    }

    sendPasswordResetEmail(authentication, email)
      .then(() => {
        addToast({
          severity: "success",
          message: "Foi enviado um email com as instruções de recuperação.",
        });
      })
      .catch((err) => {
        console.log("handleForgotUser error", err);
        addToast({
          severity: "error",
          message: AuthErrorHandler[[err.code]],
        });
      });
  }

  async function getUserByID(id) {
    const usersRef = doc(db, "users", id);
    const userSnap = await getDoc(usersRef);
    const user = userSnap.data();

    return user;
  }

  function signInUser(email, password) {
    if (isStringEmpty(email)) {
      const status = {
        status: false,
        message: "O campo email não foi preenchido",
      };
      return status;
    }
    if (isStringEmpty(password)) {
      const status = {
        status: false,
        message: "O campo senha não foi preenchido",
      };
      return status;
    }

    return signInWithEmailAndPassword(authentication, email, password)
      .then(async (re) => {
        setIsLogged(true);
        setKeyLocalStorage("UID", re.user.uid);
        const currentUser = await getUserByID(re.user.uid);
        setUser(currentUser);
        const status = {
          user: currentUser,
          status: true,
        };
        return status;
      })
      .catch((err) => {
        const status = {
          status: false,
          message: AuthErrorHandler[err.code],
          err,
        };

        console.log("signInUser error", status);
        return status;
      });
  }

  async function RegisterUser({ email, password, user }) {
    return createUserWithEmailAndPassword(authentication, email, password)
      .then(async (re) => {
        const newUser = {
          uid: re.user.uid,
          ...user,
        };
        try {
          await setDoc(doc(db, "users", re.user.uid), newUser);

          setKeyLocalStorage("UID", re.user.uid);

          setUser(newUser);
          return true;
        } catch (err) {
          addToast({
            severity: "error",
            message:
              "Houve um erro ao cadastrar o usuario. Tente novamente mais tarde",
          });
          return false;
        }
      })
      .catch((err) => {
        addToast({
          severity: "error",
          message: AuthErrorHandler[err.code],
        });
        return false;
      });
  }

  useEffect(() => {
    const executeAsync = async () => {
      const value = await AsyncStorage.getItem("@isMenuHide");
      if (value !== null) {
        setIsMenuHide(JSON.parse(value));
      } else {
        await AsyncStorage.setItem("@isMenuHide", "false");
      }
    };
    executeAsync();
  }, []);

  useEffect(() => {
    const unsubscribe = authentication.onAuthStateChanged(async (user) => {
      if (user) {
        setIsLogged(true);
        setKeyLocalStorage("UID", user.uid);
        const loggedUser = await getUserByID(user.uid);
        setUser(loggedUser);
      } else {
        setIsLogged(false);
      }
    });

    return () => {
      unsubscribe();
    };
  }, []);

  return (
    <AuthContext.Provider
      value={{
        user,
        RegisterUser,
        signInUser,
        logout,
        handleForgotUser,
        isLogged,
        getUserByID,
        isMenuHide,
        setIsMenuHide,
      }}
    >
      {props.children}
    </AuthContext.Provider>
  );
}
