...
export default function EditCliente() {
  ...
  const { updateCliente, selectedCliente } = useClientes();
  ...
  const formik = useFormik({
    initialValues: {
      ...
      phoneNumber: selectedCliente.phoneNumber ?? "",
      cpf: selectedCliente.cpf !== "" ? cpf(selectedCliente.cpf) : "",
      zipcode:
        selectedCliente.zipcode !== "" ? cep(selectedCliente.zipcode) : "",
      street: selectedCliente.street ?? "",
      number: selectedCliente.number ?? "",
      neigborhood: selectedCliente.neigborhood ?? "",
      city: selectedCliente.city ?? "",
      state: selectedCliente.state ?? "",
    },
  ...
  useEffect(() => {
    const executeAsync = async () => {
      const createdBy = await getUserByID(selectedCliente.createdBy);
      const updatedBy = await getUserByID(selectedCliente.updatedBy);
      ...
      >
        ...
        <Stack>
          <Typography>
            ...
            {dayjs().from(dayjs(new Date(selectedCliente?.createdAt)), true)}
          </Typography>
          <Typography>
            <b>Atualizado por:</b> {updatedDataInfo?.updatedBy?.name ?? ""} em{" "}
            {dayjs().from(dayjs(new Date(selectedCliente?.updatedAt)), true)}
          </Typography>
        </Stack>
      </Box>
    </DrawerComponent>
